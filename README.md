...
### 2️⃣ MVP 지향적 프론트엔드 예외 및 UX 방어 (Vanilla JS)
* **문제**: 백엔드 핵심 비즈니스 로직(주문/결제 아키텍처 튜닝)에 집중하는 과정에서, 아직 구현되지 않은 기능(주문 내역, 장바구니 등)에 접근할 때 404 에러 화면이 노출되어 유저 경험을 해치는 이슈 발생.
* **해결**: 
  * 공통 내비게이션 영역에 Vanilla JS 기반의 전역 인터셉터 역할을 하는 방어 스크립트(`ready-btn`)를 구현하여 에러 화면 이동을 차단(e.preventDefault)했습니다.
  * 유쾌한 "시스템 튜닝 안내 얼럿"을 띄워 유저에게 에러가 아닌 '준비 중인 피처'임을 명시하여, **가장 빠른 MVP 검증**이라는 스타트업 개발 철학에 맞는 프론트엔드 UX 대응을 완료했습니다.
